        ...
        if (checkCollision(this, player)) {
          ...
          if (
            ...
            const startTiles = gameStore.gameObjects.filter(
              (obj: GameObject) => obj.type === GameObjectType.TILE && obj.tileType === TileType.START.toString()
            );
...
function createPlayer(
  ...
  const player: GameObject = {
    ...
    spriteFrameCount: 4,        // 4 frames in the idle animation
    spriteFrameWidth: 32,       // Each frame is 32px wide
    spriteFrameHeight: 32,      // Each frame is 32px high
    spriteAnimationSpeed: 8,    // 8 frames per second
    spriteAnimationTime: 0,     // Start at the beginning of animation
    ...
    update(deltaTime: number) {
      ...
      if (this.velocity!.x !== 0) {
        this.spriteKey = 'player_run';
      } else if (this.isJumping) {
        this.spriteKey = 'player_jump';
      } else {
        this.spriteKey = 'player_idle';
      }
